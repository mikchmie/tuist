...
| **Martwy kod**: `xcodebuildDestination`, `simulatorPlatform`, `xcodebuildPlatform`, `hasConcreteDevice`, `xcodebuildDestinationParameter` (`:2239-2309`, ~70 linii) i `passedValue` (`:2170-2175`) | Nieosiągalne z żadnego call site w tym pliku (zweryfikowane grepem po `cli/Sources` i `cli/Tests`) | Nie jest to luka testowa — to zalegający kod do usunięcia |
...
| **`cli/Sources/TuistKit/Services/XcodeBuild/XcodeBuildTestCommandService.swift`** | **26/73 (36%)** | **Sprzężenie architektoniczne — "ukryty bliźniak"**, duplikuje ~14 z 27 zależności `TestService` |
...
**Najbezpieczniejszy szew:** granica schematu OpenAPI `RunsTest`/`ShardPlan` jest już dobrze zaprojektowana — `TestService.swift` nigdy nie dotyka `Components.Schemas.*` bezpośrednio, tylko przez `CreateTestServicing`/`UploadResultBundleServicing`. Ryzyko tam jest po stronie serwera (churn schematu wymuszający regenerację), nie w projekcie CLI.
...
4. **Martwy kod inflacyjący plik.** `xcodebuildDestination`, `simulatorPlatform`, `xcodebuildPlatform`, `hasConcreteDevice`, `xcodebuildDestinationParameter` (`:2239-2309`, ~70 linii) i `passedValue` (`:2170-2175`, zduplikowany identycznie w 3 innych plikach: `XcodeBuildTestCommandService.swift`, `XcodeBuildBuildCommandService.swift`, `XcodeBuildArgumentParser.swift`) są nieosiągalne z żadnego call site — kandydaci do prostego usunięcia, zero ryzyka regresji.
...
- **Granica generowanego kodu jest dobrze zaprojektowana**: `TestService.swift` nigdy nie dotyka bezpośrednio `Components.Schemas.*`/`Operations.*` z wygenerowanego klienta OpenAPI — zawsze przez protokoły serwisowe (`CreateTestServicing`, `UploadResultBundleServicing` itd.). To ogranicza blast radius zmian schematu do warstwy serwisowej, nie do orkiestratora.
